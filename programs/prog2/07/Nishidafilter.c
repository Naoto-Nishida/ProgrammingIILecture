...
void GaussianFilter(ImageData *imgIn, ImageData *imgOut){
  ...
  unsigned char img[_w*_h][3];
  unsigned char img2[_w*_h];
  ...
  if(is_color){
	...
	*(imgOut->data + i*3) = (unsigned char)((2*((int)img[i+left][1]+(int)img[i+right][1]+(int)img[i+upper][1]+(int)img[i+lower][1])+4*((int)img[i][1])+((int)img[i+upper+left][1]+(int)img[i+upper+right][1]+(int)img[i+lower+left][1]+(int)img[i+lower+right][1]))/16);
	*(imgOut->data + i*3) = (unsigned char)((2*((int)img[i+left][2]+(int)img[i+right][2]+(int)img[i+upper][2]+(int)img[i+lower][2])+4*((int)img[i][2])+((int)img[i+upper+left][2]+(int)img[i+upper+right][2]+(int)img[i+lower+left][2]+(int)img[i+lower+right][2]))/16);
...
//printf("%d  ", i);
    ...
    for(yi=0; yi< _h; yi++ ){
      for(xi=0; xi< _w; xi++){
	int i = xi + yi*_w;
	int upper=i-_w, lower=i+_w, right=i+1, left=i-1;
